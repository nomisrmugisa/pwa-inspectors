#!/usr/bin/env python3
"""Replace getInspectionIdForFacility with corrected version."""

import re

# Read the original file
with open('src/hooks/useAPI.js', 'r', encoding='utf-8') as f:
    content = f.read()

# New implementation
new_method = '''  async getInspectionIdForFacility(orgUnitId) {
...
pattern = r'  async getInspectionIdForFacility\(orgUnitId\) \{[\s\S]*?^\s{2}\}'

# Replace
new_content = re.sub(pattern, new_method, content, flags=re.MULTILINE)

# Write back
with open('src/hooks/useAPI.js', 'w', encoding='utf-8', newline='\r\n') as f:
    f.write(new_content)

print("✅ Successfully updated getInspectionIdForFacility method")
