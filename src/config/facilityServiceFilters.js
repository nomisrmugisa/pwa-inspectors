  ...
  "Gynae Clinics": {
    "ORGANISATION AND MANAGEMENT: Inspection": {
      "showOnly": [
        ...
        "Does the facility comply with regulatory requirements for Permission to operate/set up?",
        ...
        "Does the facility comply with regulatory requirements for",
        ...
        "Clinic provide Education credentials",
        "Clinic provide A signed confidentiality clause",
        "Clinic provide Evidence of orientation and induction",
        "Clinic provide Evidence of continuous professional development(CPD)",
    ...
    "FACILITY: Consultation Room Inspection": {
      "showOnly": [
        ...
        "Does the consultation room have condoms?",
        "Does the consultation room have cotton swabs soaked in alcohol?",
        "Does the consultation room have a diagnostic set?",
        "Does the consultation room have a drip stand?",
        "Does the consultation room have an emergency tray?",
        "Does the consultation room have an examination gynaecological bed/couch with straps, covered with linen?",
...
/**
 * Check if a data element should be shown for a specific service/facility type
 * @param {string} dataElementName - The name of the data element
 * @param {string} sectionName - The name of the section
 * @param {string} facilityType - The type of facility
 * @param {boolean} isComment - Whether this is a comment field
 * @param {string} mainDataElementName - The name of the main data element (for comment fields)
 * @returns {boolean} - Whether the data element should be shown
 */
export function shouldShowDataElementForService(dataElementName, sectionName, facilityType, isComment = false, mainDataElementName = null) {
  // If no facility type is specified, show all elements
  if (!facilityType) {
    return true;
  }

  // Get the facility configuration
  const facilityConfig = facilityServiceFilters[facilityType];
  if (!facilityConfig) {
    return true; // Show all if facility type not found
  }

  // Get the section configuration
  const sectionConfig = facilityConfig[sectionName];
  if (!sectionConfig || !sectionConfig.showOnly) {
    return true; // Show all if section not found
  }

  // For comment fields, check against the main data element name
  const elementToCheck = isComment && mainDataElementName ? mainDataElementName : dataElementName;

  // Check if the data element is in the showOnly list
  return sectionConfig.showOnly.includes(elementToCheck);
}
