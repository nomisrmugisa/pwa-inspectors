...
export const shouldShowSection = (sectionName, facilityClassification) => {
  ...
  // Aliases to bridge legacy config names to CSV-driven names
  ...
  // If not found, try alias mapping (e.g. SERVICES PROVIDED -> SERVICE PROVIDED)
  if (shouldShow === undefined) {
    const aliasKey = SECTION_NAME_ALIASES[sectionNameUpper];
    if (aliasKey && facilityConfig[aliasKey] !== undefined) {
      shouldShow = facilityConfig[aliasKey];
    }
  }
