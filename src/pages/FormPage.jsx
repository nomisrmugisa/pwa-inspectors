...
function FormSection({ section, formData, onChange, errors, serviceSections, loadingServiceSections, readOnlyFields = {}, getCurrentPosition, formatCoordinatesForDHIS2, facilityClassifications = [], loadingFacilityClassifications = false, inspectionInfoConfirmed = false, setInspectionInfoConfirmed = () => { }, areAllInspectionFieldsComplete = () => false, showDebugPanel = false, getCurrentFacilityClassification = () => null, facilityType = null, manualSpecialization = null, onCommentChange, comments = {}, selectedServiceDepartments = [], activeSectionId = null, onNextSection = null, onPreviousSection = null, hasNextSection = false, hasPreviousSection = false, targetSubsectionId = null, onSubsectionNavigated = null }) {
  ...
  // Determine the facility classification used for filtering.
  // Prefer the shared helper (which prioritizes manual "Choose Category" selection),
  // fall back to the FormSection's facilityType prop if needed.
  const filteringFacilityType = (typeof getCurrentFacilityClassification === 'function'
    ? getCurrentFacilityClassification()
    : null) || facilityType;
  ...
  const filterDataElements = (dataElements) => {
    ...
    if (!filteringFacilityType || !dataElements || !Array.isArray(dataElements)) {
      ...
      const results = await Promise.all(
        ...
        section.dataElements.map(async (psde, idx) => {
          ...
          if ((sectionName || '').toUpperCase().includes('ULTRASOUND')) {
            const debugRes = await shouldShowDataElementForService(displayName, filteringFacilityType, sectionName);
...
const DEPARTMENT_SECTION_MAPPING = {
  ...
  'SECTION A-ORGANISATION AND MANAGEMENT': ['ORGANISATION AND MANAGEMENT'], // Fix for CSV artifact
  ...
  'FACILITY - PROCEDURE ROOM': ['FACILITY-PROCEDURE ROOM'],
  'FACILITY- PROCEDURE ROOM': ['FACILITY-PROCEDURE ROOM'],
  'FACILITY-PROCEDURE ROOM-': ['FACILITY-PROCEDURE ROOM'], // Fix for CSV artifact
  'FACILITY-CONSULTATION/ TREATMENT ROOM--': ['CONSULTATION', 'TREATMENT'], // Fix for CSV artifact
  'FACILITY-RECEPTION/WAITING AREA--': ['RECEPTION', 'WAITING AREA'], // Fix for CSV artifact
  'FACILITY-SCREENING ROOM--': ['SCREENING ROOM'], // Fix for CSV artifact
  ...
  'TOILET FACILITIES HOSPITAL': ['TOILET FACILITIES HOSPITAL'],
  'RESUSCITATION SERVICES': ['RESUSCITATION SERVICES'],
  ...
  // Hospital-specific facility sections (new DHIS2 sections)
  'FACILITY GOVERNANCE AND MANAGEMENT': ['FACILITY GOVERNANCE AND MANAGEMENT'],
  "NURSES' STATION": ["NURSES' STATION"],
  'IN PATIENT ADMISSION ROOMS': ['IN PATIENT ADMISSION ROOMS'],
  'OFFICE FOR THE MANAGER': ['OFFICE FOR THE MANAGER'],
  'EMERGENCY EQUIPMENT': ['EMERGENCY EQUIPMENT'],
  'OUT PATIENT SERVICE': ['OUT PATIENT SERVICE'],
  'OBSTETRICS AND GYNAECOLOGY': ['OBSTETRICS AND GYNAECOLOGY'],
  'CENTRAL SUPPLY / STERILIZATION': ['CENTRAL SUPPLY / STERILIZATION'],
  'PHARMACY': ['PHARMACY'],
  'RADIOLOGY (MEDICAL IMAGING; X-RAY DEPARTMENT)': ['RADIOLOGY (MEDICAL IMAGING; X-RAY DEPARTMENT)'],
  'PHYSIOTHERAPY CARE-A': ['PHYSIOTHERAPY CARE-A'],
  'PHYSIOTHERAPY CARE-B': ['PHYSIOTHERAPY CARE-B'],
  'PHYSIOTHERAPY CARE': ['PHYSIOTHERAPY CARE'], // Added for updated CSV
  'ENGINEERING / MAINTENANCE SERVICES': ['ENGINEERING / MAINTENANCE SERVICES'],

  // Catch-all
  'OTHER': [], // OTHER shows all sections

  // Missing Hospital Mappings (Fix for Visibility)
  'ACCIDENT & EMERGENCY AND RESUSCITATION SERVICES': ['ACCIDENT & EMERGENCY AND RESUSCITATION SERVICES'],
  'ADMINISTRATIVE SERVICES': ['ADMINISTRATIVE SERVICES'],
  'CENTRAL SUPPLIES AND STERILISATION DEPARTMENT (CSSD)': ['CENTRAL SUPPLIES AND STERILISATION DEPARTMENT (CSSD)'],
  'COMBINED GENERAL MEDICAL/ SURGICAL/PAEDIATRIC WARDS': ['COMBINED GENERAL MEDICAL/ SURGICAL/PAEDIATRIC WARDS'],
  'CRITICAL CARE UNIT (HIGH CARE)': ['CRITICAL CARE UNIT (HIGH CARE)'],
  'CUSTOMER CARE, RIGHTS AND SATISFACTION': ['CUSTOMER CARE, RIGHTS AND SATISFACTION'],
  'EYE CLINIC': ['EYE CLINIC'],
  'FACILITY ENVIRONMENT': ['FACILITY ENVIRONMENT'],
  'FACILITY RESUSCITATION SERVICES': ['FACILITY RESUSCITATION SERVICES'],
  'FOOD SERVICE AND KITCHEN': ['FOOD SERVICE AND KITCHEN'],
  'GENERAL MEDICAL WARDS': ['GENERAL MEDICAL WARDS'],
  'HUMAN RESOURCE MANAGEMENT': ['HUMAN RESOURCE MANAGEMENT'],
  'INFECTION PREVENTION AND CONTROL': ['INFECTION PREVENTION AND CONTROL'],
  'LABORATORY': ['LABORATORY'],
  'LAUNDRY SERVICES': ['LAUNDRY SERVICES'],
  'PAEDIATRIC CARE/ SPECIALTIES AND WARDS/ NEONATOLOGY': ['PAEDIATRIC CARE/ SPECIALTIES AND WARDS/ NEONATOLOGY'],
  'PSYCHIATRIC SERVICES AND WARDS': ['PSYCHIATRIC SERVICES AND WARDS'],
  'SECTION 25': ['SECTION 25'],
  'SOCIAL WORK': ['SOCIAL WORK'],
  'SPEECH THERAPY': ['SPEECH THERAPY'],
  'SURGICAL /ORTHOPAEDIC WARDS': ['SURGICAL /ORTHOPAEDIC WARDS'],

  // New sections from updated checklist-final.csv
  'DENTAL': ['DENTAL'],
  'DIETETICS': ['DIETETICS'],
  'ELISA': ['ELISA'],
  'EQUIPMENT AND HEALTHCARE TECHNOLOGY': ['EQUIPMENT AND HEALTHCARE TECHNOLOGY'],
...
function FormPage() {
  ...
  const handleSpecializationChange = (selectedSpecialization) => {
    ...
    if (!isLoadingFromIndexedDB) {
      ...
      // window.updateSelectedServiceDepartments is defined in the useEffect below
      if (window.updateSelectedServiceDepartments) {
